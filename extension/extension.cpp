...
KHook::Return<CBaseEntity *> Hook_GiveNamedItem(CBaseEntity *pEntity, char const *szClassname, int iSubType, CEconItemView *cscript, bool b);
KHook::Return<CBaseEntity *> Hook_GiveNamedItem_Post(CBaseEntity *pEntity, char const *szClassname, int iSubType, CEconItemView *cscript, bool b);
KHook::Return<void> Hook_ClientPutInServer(IServerGameClients *pServerGameClients, edict_t *pEntity, char const *playername);

KHook::Virtual<CBaseEntity, CBaseEntity *, char const *, int, CEconItemView *, bool>
	g_GiveNamedItemHook(Hook_GiveNamedItem, Hook_GiveNamedItem_Post);
KHook::Virtual<IServerGameClients, void, edict_t *, char const *>
	g_ClientPutInServerHook(&IServerGameClients::ClientPutInServer, nullptr, Hook_ClientPutInServer);
...
bool g_bHookedPlayer = false;
bool g_bHookedBot = false;
bool g_bHookedClientPutInServer = false;
...
KHook::Return<CBaseEntity *> Hook_GiveNamedItem(CBaseEntity *pEntity, char const *szClassname, int iSubType, CEconItemView *cscript, bool b)
{
	...
	if (cscript == NULL || szClassname == NULL)
	{
		...
		g_pSM->LogMessage(myself, "(cscript == NULL || szClassname == NULL), ignore");
		...
		case Pl_Changed:
			{
				...
				CBaseEntity *pNewItem = g_GiveNamedItemHook.CallOriginal(pEntity, finalitem, iSubType, &newitem, ((pScriptedItemOverride->m_bFlags & FORCE_GENERATION) == FORCE_GENERATION));
				return { KHook::Action::Supersede, pNewItem };
			}
		case Pl_Handled:
		case Pl_Stop:
			{
...
KHook::Return<CBaseEntity *> Hook_GiveNamedItem_Post(CBaseEntity *pEntity, char const *szClassname, int iSubType, CEconItemView *cscript, bool b)
{
	CBaseEntity *player = pEntity;

	CBaseEntity *pItemEntiy = *reinterpret_cast<CBaseEntity **>(KHook::GetCurrentValuePtr());

	if (!player || !szClassname || !cscript || !pItemEntiy)
		return { KHook::Action::Ignore, pItemEntiy };
...
KHook::Return<void> Hook_ClientPutInServer(IServerGameClients *pServerGameClients, edict_t *pEntity, char const *playername)
{
	...
	if(pEntity->m_pNetworkable)
	{
		...
		if (HookTFBot.GetBool() && strcmp(pEntity->GetClassName(), "tf_bot") == 0)
		{
			...
			if (!HookTFBot.GetBool() && g_bHookedClientPutInServer) {
				g_ClientPutInServerHook.Remove(gameclients);
				g_bHookedClientPutInServer = false;
#ifdef TF2ITEMS_DEBUG_HOOKING
		...
		if (g_bHookedClientPutInServer && g_bHookedPlayer && g_bHookedBot) {
			g_ClientPutInServerHook.Remove(gameclients);
			g_bHookedClientPutInServer = false;
#ifdef TF2ITEMS_DEBUG_HOOKING
			g_pSM->LogMessage(myself, "ClientPutInServer unhooked.");
...
bool TF2Items::SDK_OnLoad(char *error, size_t maxlen, bool late) {
	...
	if (!g_pGameConf->GetOffset("GiveNamedItem", &iOffset))
	{
		...
		g_GiveNamedItemHook.Configure(iOffset);
	...
	if (!g_bHookedPlayer)
	{
		...
		g_ClientPutInServerHook.Add(gameclients);
		g_bHookedClientPutInServer = true;
...
static cell_t TF2Items_GiveNamedItem(IPluginContext *pContext, const cell_t *params)
{
	...
	tempItem = g_GiveNamedItemHook.CallOriginal(pEntity, strWeaponClassname, 0, &hScriptCreatedItem, ((pScriptedItemOverride->m_bFlags & FORCE_GENERATION) == FORCE_GENERATION));
